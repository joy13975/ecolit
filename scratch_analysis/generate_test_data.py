...
class TestDataGenerator:
    ...
    def calculate_grid_flow(self, df: pd.DataFrame) -> list[float]:
        ...
        for hour in range(24):
            import_kwh = (
                df.loc[hour, "grid_import"] if not pd.isna(df.loc[hour, "grid_import"]) else 0
            )
            export_kwh = (
                df.loc[hour, "grid_export"] if not pd.isna(df.loc[hour, "grid_export"]) else 0
            )
            ...
            export_w = (
                abs(export_kwh) * 1000
            )  # Export is typically negative, make positive for calculation
    ...
    def interpolate_to_10s(
        self, hourly_data: list[float], date: str
    ...
    def generate_test_data(self, date: str, output_dir: Path) -> None:
        ...
        for hour in range(24):
            charging = (
                df.loc[hour, "battery_charging"]
                if not pd.isna(df.loc[hour, "battery_charging"])
                else 0
            )
            discharging = (
                df.loc[hour, "battery_discharge"]
                if not pd.isna(df.loc[hour, "battery_discharge"])
                else 0
            )
        ...
        for i in range(len(timestamps)):
            metrics = EnergyMetrics(
                ...
                home_battery_soc=round(
                    max(0, min(100, home_battery_soc_10s[i])), 1
                ),  # Clamp 0-100%
    ...
    def generate_summary(self, date: str, test_data: list[EnergyMetrics], output_dir: Path) -> None:
        ...
        export_periods = len(
            [g for g in grid_flows if g < -50]
        )  # Exporting >50W (triggers EV charge increase)
        import_periods = len(
            [g for g in grid_flows if g > 0]
        )  # Importing (triggers EV charge decrease)
        ...
        print(
            f"  Grid flow: {summary['grid_flow_stats']['percent_export_opportunities']}% export opportunities (>50W)"
        )
        print(
            f"  Home Battery SOC: {summary['home_battery_soc_stats']['min_percent']:.1f}% - {summary['home_battery_soc_stats']['max_percent']:.1f}%"
        )
