...
class EnergyAnalyzer:
    ...
    def _parse_graph_data(self, graph_data: list[dict]) -> dict[str, list]:
        ...
        for series in graph_data:
            ...
            if clean_name in column_mapping:
                ...
                print(
                    f"  - {clean_name} → {col_name}: {len([x for x in clean_data if not np.isnan(x) if isinstance(x, float)])} valid values"
                )
    ...
    def analyze_daily_patterns(self) -> None:
        ...
        for date, df in self.daily_data.items():
            ...
            battery_charge_total = abs(
                totals.get("battery_charging_kwh", 0)
            ...
            start_soc = (
                df["battery_soc_pct"].iloc[0] if not pd.isna(df["battery_soc_pct"].iloc[0]) else 0
            )
            end_soc = (
                df["battery_soc_pct"].iloc[-1] if not pd.isna(df["battery_soc_pct"].iloc[-1]) else 0
            )
            ...
            print(
                f"📈 Consumption Spikes:  {spike_total:6.1f} kWh over {spike_hours} hours (includes Tesla + other)"
            )
            print("   (User reported Tesla: ~13-14kWh for these days)")
            ...
            print(
                f"🎯 Self-consumption:    {self_consumption_pct:6.1f}% ({self_consumption:.1f}/{solar_total:.1f} kWh)"
            )

            # Grid dependency
            grid_dependency = (
                grid_import_total / consumption_total * 100 if consumption_total > 0 else 0
            )
            ...
            end_of_solar_soc = (
                df["battery_soc_pct"].iloc[end_of_solar_hour]
                if end_of_solar_hour < len(df)
                else end_soc
            )

            print(
                f"🌅 Solar ends ~{end_of_solar_hour:02d}:00, Battery SOC: {end_of_solar_soc:.0f}%"
            )
    ...
    def compare_optimization_strategies(self) -> None:
        ...
        for date, results in results_df.iterrows():
            print(
                f"{date:<6} {results['solar_total']:>6.1f} {results['tesla_charging']:>6.1f} "
        ...
        for date, results in results_df.iterrows():
            ...
            if charging_periods:
                ...
                if early_ratio > 0.5:
                    print("  ⚠️  Heavy early charging → Lower end-of-day SOC")
                else:
                    print("  ✅ Later charging → Better end-of-day SOC")
    ...
    def create_visualizations(self) -> None:
        ...
        for idx, (date, df) in enumerate(self.daily_data.items()):
            ...
            ax.bar(
                hours, df["solar_generation_kwh"], alpha=0.7, label="Solar Generation", color="gold"
            )
        ...
        plt.tight_layout()
        plt.savefig(
            self.data_dir.parent / "energy_analysis_charts.png", dpi=300, bbox_inches="tight"
        )
    ...
    def generate_optimization_recommendations(self) -> None:
        ...
        if best_periods:
            ...
            print("   - Tesla charging: 0A")
            print("   - Let home battery charge from solar first")
            print("   - Target: Reach 80-90% home battery SOC")

            print(f"\n2. **Surplus Charging Phase ({earliest_hour:02d}-{latest_hour:02d}h):**")
            print("   - Tesla charging: 6-20A based on surplus")
            print("   - Calculate: PV surplus = Solar - House - Grid_import")
            print("   - Target: Use 13-15kWh that would otherwise export")

            print(f"\n3. **End-of-Day Phase ({latest_hour:02d}h+):**")
            print("   - Tesla charging: Reduce as solar drops")
            print("   - Target: Maintain home battery >70% for evening")
        ...
        print("- Achieve end-of-solar battery SOC: >85%")
        print("- Tesla charging: 13-15kWh daily (your typical export amount)")
        print("- Grid dependency: <10%")

        print("\n🔧 IMPLEMENTATION PARAMETERS:")
        print("-" * 35)
        print("```yaml")
        print("tesla_charging:")
        print("  max_amperage: 20  # Your breaker limit")
        print("  min_home_battery_soc: 70  # Before starting Tesla charge")
        print("  surplus_threshold: 1500   # Minimum 1.5kW surplus")
        print("  target_daily_kwh: 14      # Target Tesla charging")
        print("  charging_window:")
...
def main():
    ...
    data_dir = base_dir / "data" / "omron" / "daily"
