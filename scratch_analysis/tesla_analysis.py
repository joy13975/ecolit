...
def load_tesla_data():
    ...
    tesla_dir = base_dir / "data" / "tesla" / "wall_connector"
...
def analyze_tesla_patterns():
    ...
    for date in sorted(tesla_data.keys()):
        ...
        if len(charging_df) > 0:
            ...
            print(
                f"⚡ Power levels: {min_power:.1f}kW → {avg_power:.1f}kW avg → {max_power:.1f}kW max"
            )
...
def compare_tesla_vs_hems():
    ...
    base_dir = Path(__file__).parent.parent
    hems_dir = base_dir / "data" / "omron" / "daily"
    ...
    for date in sorted(tesla_data.keys()):
        ...
        print("Hour Tesla  HEMS   Baseline  Spike")
        print("-" * 35)
        ...
        for hour in range(6, 18):  # Focus on daylight hours
            ...
            if tesla_h > 0.1 or spike_h > 0.5:  # Show significant periods
                print(
                    f"{hour:02d}h  {tesla_h:5.1f}  {hems_h:5.1f}    {baseline_h:5.1f}    {spike_h:5.1f}"
                )
...
def identify_charging_strategies():
    ...
    for date in sorted(tesla_data.keys()):
        ...
        print(f"\n📅 {date} Strategy:")
        print(
            f"🕐 Charging window: {start_hour:02d}:xx → {end_hour:02d}:xx ({end_hour - start_hour + 1}h span)"
        )
        ...
        if gaps:
            ...
            print("▶️  Continuous charging")

    # Compare strategies
    print("\n📊 STRATEGY COMPARISON:")
    print("Date  Start  End  Duration  Total   Max     Strategy")
    print("      Hour   Hour   (h)     (kWh)  (kW)    Pattern")
    print("-" * 60)
    ...
    for date, strategy in strategies.items():
        ...
        print(
            f"{date}   {strategy['start_hour']:02d}     {strategy['end_hour']:02d}      "
