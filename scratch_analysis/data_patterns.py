...
def analyze_patterns():
    ...
    for date in sorted(data.keys()):
        ...
        end_soc = (
            ...
            if not pd.isna(df["battery_soc"].iloc[-1])
            else df["battery_soc"].dropna().iloc[-1]
        )
        ...
        solar_end_soc = (
            df["battery_soc"].iloc[solar_end_hour] if solar_end_hour < len(df) else end_soc
        )
        ...
        if len(high_consumption_hours) > 0:
            ...
            print(
                f"Peak consumption:     {peak_consumption:.1f} kWh at {peak_hour}h (battery: {peak_soc:.0f}%)"
            )
    ...
    for date in sorted(data.keys()):
        ...
        solar_end_soc = (
            df["battery_soc"].iloc[solar_end_hour]
            if solar_end_hour < len(df)
            else df["battery_soc"].iloc[-1]
        )
        grid_export = abs(df["grid_export"].sum())
        grid_import = df["grid_import"].sum()

        print(
            f"{date}: End-of-solar SOC={solar_end_soc:3.0f}%, Export={grid_export:4.1f}kWh, Import={grid_import:4.1f}kWh"
        )

    # Key observation
    print("\n💡 KEY OBSERVATIONS (No Assumptions):")
    print("- All days: ~13-14kWh Tesla charging (user confirmed)")
    print("- 0827: Highest end-of-solar battery SOC (~80%)")
    print("- 0827: Some grid export (2.0kWh) but best evening battery reserve")
    print("- 0825/0826: Lower end-of-solar SOC (46%, 35%) despite no export")
    print("- Pattern: Higher end-of-solar SOC = better evening energy autonomy")

    # What differs between days?
    print("\n🤔 WHAT'S DIFFERENT ABOUT 0827?")
    print("- Similar Tesla charging amount (~13-14kWh)")
    print("- Different timing of high-consumption periods?")
    print("- Different balance between battery vs Tesla priority?")
    print("- Need to examine HOURLY patterns, not just totals")
