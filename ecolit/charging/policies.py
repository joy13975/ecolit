...
class EcoPolicy(ChargingPolicy):
    ...
    def calculate_target_amps(self, current_amps: int, metrics: EnergyMetrics) -> int:
        ...
        if metrics.grid_power_flow < -self.export_threshold:
            ...
            logger.debug(f"ECO: Exporting {abs(metrics.grid_power_flow)}W > {self.export_threshold}W, increase to {target_amps}A")
        elif metrics.grid_power_flow >= 0:
            # Not exporting (importing or balanced) - decrease charging
            target_amps = current_amps - 2
            logger.debug(f"ECO: Grid flow {metrics.grid_power_flow}W ≥ 0, decrease to {target_amps}A")
        else:
            # Exporting but less than threshold - maintain current
            target_amps = current_amps
            logger.debug(f"ECO: Exporting {abs(metrics.grid_power_flow)}W < {self.export_threshold}W, maintain {target_amps}A")
...
class HurryPolicy(ChargingPolicy):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.export_threshold = config.get("eco", {}).get("export_threshold", 50)  # Reuse ECO threshold
    ...
    def calculate_target_amps(self, current_amps: int, metrics: EnergyMetrics) -> int:
        ...
        if metrics.grid_power_flow < -self.export_threshold:
            # Exporting > threshold - can increase charging
            target_amps = current_amps + 1
            logger.debug(f"HURRY: Exporting {abs(metrics.grid_power_flow)}W > {self.export_threshold}W, increase to {target_amps}A")
        elif metrics.grid_power_flow <= self.max_import:
            # Importing but within limit - can still increase (but more cautiously)
            target_amps = current_amps + 1
            logger.debug(f"HURRY: Grid flow {metrics.grid_power_flow}W ≤ {self.max_import}W limit, increase to {target_amps}A")
        else:
            # Importing too much - decrease charging
            target_amps = current_amps - 2
            logger.debug(f"HURRY: Importing {metrics.grid_power_flow}W > {self.max_import}W limit, decrease to {target_amps}A")
...
def create_policy(policy_name: str, config: dict[str, Any]) -> ChargingPolicy:
    ...
    if not policy_class:
        raise ValueError(f"Unknown charging policy: {policy_name}. Available: {list(policies.keys())}")
