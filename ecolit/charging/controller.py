...
from .policies import ChargingPolicy, EnergyMetrics, create_policy
...
class EVChargingController:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        logger.info(f"EV charging controller initialized: policy={self.policy.get_name()}, max_amps={self.max_amps}")
    ...
    def calculate_charging_amps(self, metrics: EnergyMetrics) -> int:
        ...
        if self.should_adjust():
            ...
            if self.target_amps != self.current_amps:
                logger.info(f"🔌 EV CHARGING: {self.policy.get_name()} policy → {self.current_amps}A to {self.target_amps}A")
                self.current_amps = self.target_amps
                self.last_adjustment_time = current_time
            else:
                logger.debug(f"EV charging: {self.policy.get_name()} policy maintains {self.current_amps}A")
        else:
    ...
    def get_status_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "next_adjustment_in": max(0, self.adjustment_interval - (current_time - self.last_adjustment_time)),
        }
