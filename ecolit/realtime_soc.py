...
class SoCEstimate:
    ...
    confidence: float
    source: str
    official_soc: Optional[float] = None
    time_since_official_hours: Optional[float] = None
...
class RealtimeSoCEstimator:
    ...
    def __init__(self, battery_capacity_kwh: float):
        ...
        logger.info(f"🔋 Real-time SoC Estimator initialized (capacity: {battery_capacity_kwh:.1f}kWh)")
        
    def update_official_soc(self, soc_percent: float, timestamp: Optional[datetime] = None) -> None:
        ...
        if self.official_soc is not None and self.official_soc_time is not None:
            ...
            if abs(soc_change) >= 0.1:  # Only log significant changes
                logger.info(f"📊 Official Home Battery SoC: {self.official_soc:.1f}% → {soc_percent:.1f}% "
    ...
    def update_power(self, power_w: float, timestamp: Optional[datetime] = None) -> None:
        ...
        if self.last_power_update is not None and len(self.power_history) >= 2:
            ...
            if time_elapsed_hours > 0:
                ...
                previous_power = self.power_history[-2][1] if len(self.power_history) >= 2 else power_w
    ...
    def get_estimated_soc(self, timestamp: Optional[datetime] = None) -> SoCEstimate:
        ...
        if abs(self.cumulative_energy_wh) > 1.0:  # Only if significant energy change
            ...
            energy_confidence = max(0.5, 1.0 - abs(self.cumulative_energy_wh / (self.battery_capacity_wh * 0.05)))  # Decay with large changes
    ...
    def get_charging_info(self) -> Dict[str, Any]:
        ...
        if avg_power_w > 10 and current_estimate.estimated_soc is not None:  # Only if actively charging
