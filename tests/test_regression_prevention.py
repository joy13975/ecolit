...
from ecolit.charging.policies import EcoPolicy
from ecolit.charging.tesla_controller import TeslaChargingController
from ecolit.charging.tesla_api import TeslaVehicleData, TeslaAPIClient
...
class TestRegressionPrevention:
    ...
    @pytest.mark.asyncio
    async def test_sleeping_vehicle_triggers_wake_up_call(self, tesla_config, sleeping_vehicle_data, awake_stopped_vehicle_data):
        """REGRESSION TEST: Verify wake-up API is called when vehicle is sleeping."""
        
        with patch("ecolit.charging.tesla_api.TeslaAPIClient.wake_up") as mock_wake_up, \
             patch("ecolit.charging.tesla_api.TeslaAPIClient.poll_vehicle_data_with_wake_option") as mock_poll, \
    ...
    @pytest.mark.asyncio
    async def test_charge_commands_called_with_correct_parameters(self, tesla_config, awake_stopped_vehicle_data):
        """REGRESSION TEST: Verify Tesla API methods called with correct parameters."""
        
        with patch("ecolit.charging.tesla_api.TeslaAPIClient.poll_vehicle_data_with_wake_option") as mock_poll, \
             patch("ecolit.charging.tesla_api.TeslaAPIClient.charge_start") as mock_start, \
             patch("ecolit.charging.tesla_api.TeslaAPIClient.set_charging_amps") as mock_amps:
            ...
            for target_amps in test_cases:
                ...
                result = await controller.execute_charging_control_with_wake(target_amps=target_amps)
                ...
                mock_amps.assert_called_once_with(target_amps), f"set_charging_amps should be called with {target_amps}A"
    ...
    async def test_stop_charging_calls_correct_api(self, tesla_config):
        ...
        with patch("ecolit.charging.tesla_api.TeslaAPIClient.get_vehicle_data") as mock_get_data, \
    ...
    @pytest.mark.asyncio 
    async def test_wake_up_failure_prevents_charging_commands(self, tesla_config, sleeping_vehicle_data):
        ...
        with patch("ecolit.charging.tesla_api.TeslaAPIClient.poll_vehicle_data_with_wake_option") as mock_poll, \
            ...
            mock_amps.assert_not_called(), "set_charging_amps should not be called when wake-up fails"

    @pytest.mark.asyncio
    async def test_commands_never_sent_to_sleeping_car_without_wakeup(self, tesla_config, sleeping_vehicle_data, awake_stopped_vehicle_data):
        ...
        with patch("ecolit.charging.tesla_api.TeslaAPIClient.poll_vehicle_data_with_wake_option") as mock_poll, \
             patch("ecolit.charging.tesla_api.TeslaAPIClient.wake_up", side_effect=log_wake_up) as mock_wake_up, \
             patch("ecolit.charging.tesla_api.TeslaAPIClient.charge_start", side_effect=log_charge_start) as mock_start, \
             patch("ecolit.charging.tesla_api.TeslaAPIClient.set_charging_amps", side_effect=log_set_amps) as mock_amps:
            ...
            assert len(call_log) >= 3, f"Expected at least 3 calls (wake_up, charge_start, set_amps), got: {call_log}"
            ...
            assert wake_up_pos < charge_start_pos, f"wake_up must come before charge_start: {call_log}"
