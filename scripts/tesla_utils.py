...
async def handle_sleeping_vehicle_with_wake(
    ...
    action_description: str = "get current data"
    ...
    if is_sleeping:
        ...
        if prompt_yes_no(f"Would you like to wake the vehicle to {action_description}?", default_yes=True):
...
async def ensure_vehicle_awake_for_command(
    client: TeslaAPIClient,
    command_description: str = "send commands"
) -> bool:
    """
    ...
    """
    vehicle_data, is_awake = await handle_sleeping_vehicle_with_wake(
        client, command_description
    )
